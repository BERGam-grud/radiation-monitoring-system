...
const deviceTypes = {
    gamma: {
        name: 'Блок детектування ПАЕД гамма-випромінювання',
        icon: '📡',
        unit: 'uSv/год',
        color: '#4ade80',
        params: ['Потужність', 'Імпульси CPS']
    },
    spectro: {
        name: 'Спектрометричний блок детектування',
        icon: '🔬',
        unit: 'uSv/год',
        color: '#fbbf24',
...
// ========== КОЛЬОРОВА ІНДИКАЦІЯ ДЛЯ ГАММА-ДЕТЕКТОРІВ ==========
function getGammaColor(value) {
    if (value > 0.30) return '#ef4444';
    if (value > 0.25) return '#f59e0b';
    return '#4ade80';
}

function getGammaStatus(value) {
...
function generateDeviceData(type, count = 100) {
    ...
    switch(type) {
        case 'gamma':
            baseValue = 0.15 + Math.random() * 0.25;
            for (let i = 0; i < count; i++) {
                let dose = baseValue + (Math.random() - 0.5) * 0.15;
                let cps = dose * 100 + Math.random() * 20;
                data.push({
                    timestamp: Date.now() - (count - i) * 60000,
                    dose: Number(Math.max(0, dose).toFixed(1)),
                    cps: Math.round(Math.max(0, cps))
                });
            }
            params = {
                dose: Number(data[data.length-1].dose.toFixed(1)),
                cps: Math.round(data[data.length-1].cps),
                unit: 'мкЗв/год',
                status: 'normal'
            };
            break;
        ...
        case 'spectro':
            ...
            for (let i = 0; i < count; i++) {
                ...
                data.push({
                    timestamp: Date.now() - (count - i) * 60000,
                    activity: Number(Math.max(0, activity).toFixed(1)),
                    spectrum: generateSpectrumData()
                });
            }
            params = {
                activity: Number(data[data.length-1].activity.toFixed(1)),
                unit: 'кБк/м?',
                status: 'normal'
            };
            break;
        ...
        case 'weather':
            ...
            params = {
                temperature: Number(data[data.length-1].temperature.toFixed(1)),
                humidity: Number(data[data.length-1].humidity.toFixed(1)),
                windSpeed: Number(data[data.length-1].windSpeed.toFixed(1)),
                pressure: Number(data[data.length-1].pressure.toFixed(1)),
                unit: '°C',
                status: 'normal'
            };
...
for (let i = 1; i <= 10; i++) {
        ...
        equipment: [
            {
                ...
                logs: generateRandomLogs('Блок детектування ПАЕД гамма-випромінювання', 20),
                errors: Math.floor(Math.random() * 10),
                alarmLevels: { warning: 0.25, danger: 0.30, critical: 0.50 },
    ...
    totalTime: Number(vfuData.params.totalTime || (Math.random() * 3600 + 60).toFixed(0))
},
            {
                ...
                value: Number(weatherData.params.temperature.toFixed(1)),
                status: 'normal',
                channel: 'Температура',
...
function generateDeviceNotifications() {
    for (const device of allDevices) {
        if (device.status === 'critical') {
            addNotification('error', '🔴 КРИТИЧНО!', `${device.workplace}: ${device.name} - НЕБЕЗПЕЧНИЙ РІВЕНЬ (${device.value} ${device.unit})`, device.id);
...
function renderFilteredDevices() {
    ...
    if (tbody) {
        tbody.innerHTML = filtered.map(dev => {
            ...
            let statusText = '';
            if (dev.type === 'gamma') {
                const gammaValue = typeof dev.value === 'number' ? dev.value : 0;
                if (gammaValue > 0.30) {
                    valueColor = 'color: #ef4444;';
                    statusText = 'Критично';
                } else if (gammaValue > 0.25) {
                    valueColor = 'color: #f59e0b;';
                ...
                if (dev.status === 'critical') { valueColor = 'color: #ef4444;'; statusText = 'Критично'; }
                else if (dev.status === 'danger') { valueColor = 'color: #f87171;'; statusText = 'Небезпека'; }
                else if (dev.status === 'warning') { valueColor = 'color: #f59e0b;'; statusText = 'Попередження'; }
                else { valueColor = 'color: #4ade80;'; statusText = 'Норма'; }
            }
            ...
            return `
                <tr onclick="showDeviceDetail('${dev.id}')" style="cursor:pointer">
                    ...
                    <td>${dev.workplace}</td>
                    <td>${dev.location}</td>
                    <td style="${valueColor} font-weight: bold;">
                        ${typeof dev.value === 'number' ? dev.value.toFixed(1) : dev.value} ${dev.unit}
                    </td>
                    <td><span class="status-dot ${dev.type === 'gamma' ? getGammaStatus(dev.value) : dev.status}"></span> ${statusText}</td>
...
function initMap() {
    ...
    workplaces.forEach(workplace => {
        ...
        const deviceListHtml = workplace.equipment.map(dev => `
            <div style="display:flex; justify-content:space-between; padding:4px 0; border-bottom:1px solid #2a2a3a;">
                <span>${deviceTypes[dev.type]?.icon || '??'} ${dev.name}</span>
                <span style="color: ${dev.type === 'gamma' ? getGammaColor(dev.value) : (dev.status === 'critical' ? '#ef4444' : (dev.status === 'danger' ? '#f87171' : (dev.status === 'warning' ? '#f59e0b' : '#4ade80')))}; font-weight: bold;">
                    ${typeof dev.value === 'number' ? dev.value.toFixed(1) : dev.value} ${dev.unit}
                </span>
            </div>
        `).join('');
...
function exportDeviceExcel(deviceId) {
    ...
    switch(device.type) {
        case 'gamma':
            headers.push('Потужність (uSv/год)', 'Імпульси CPS');
            exportData = device.history.map(h => ({
                'Дата/Час': new Date(h.timestamp).toLocaleString(),
                'Потужність (uSv/год)': h.dose || h.value,
...
function exportToPDF(deviceId) {
    ...
    const printContent = `
        ...
        <body>
            ...
            <p><strong>Статус:</strong> ${device.status === 'normal' ? 'Безпечний' : (device.status === 'warning' ? 'Забруднений' : (device.status === 'danger' ? 'Небезпечний' : 'Небезпечний'))}</p>
    ...
    const modalHtml = `
        <div id="alarmLevelModal" class="modal" style="display:flex; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.7); z-index:1000; align-items:center; justify-content:center;">
            <div class="modal-content" style="background:#1e1e2e; border-radius:12px; padding:20px; width:400px; max-width:90%;">
                ...
                <div class="form-group" style="margin-bottom:20px;">
                    <label style="color:#ef4444; display:block; margin-bottom:5px;">❗ НЕБЕЗПЕЧИЙ РІВЕНЬ (темно-червоний)</label>
        ...
        <div class="monitoring-sidebar">
            <div class="filter-bar" style="flex-wrap: wrap; padding: 10px; gap: 6px; background: #1a1a2a;">
                <select id="monFilterType" class="filter-select" onchange="renderMonitoringRMList()" style="flex:1; min-width:100px; font-size:12px; padding:5px 8px;">
                    ...
                    <option value="gamma">Блок детектування ПАЕД гамма-випромінюванняи</option>
                    <option value="spectro">Спектрометри</option>
                    <option value="vfu">Повітряно-фільтрувальна установка (ПФУ) атмосферного повітря</option>
                ...
                <select id="monFilterWorkplace" class="filter-select" onchange="renderMonitoringRMList()" style="flex:1; min-width:100px; font-size:12px; padding:5px 8px;">
                    ...
                    ${workplacesList.map(w => `<option value="${w.name}">${w.name}</option>`).join('')}
                </select>
                ...
                <div class="workplace-devices show" id="mon-wp-list-${wp.id}">
                    ${equipmentList.map(dev => `
                        <div class="device-item" onclick="showMonitoringRMDetail('${dev.id}')">
                            ...
                            <div class="device-value" style="color: ${dev.type === 'gamma' ? getGammaColor(dev.value) : (dev.status === 'critical' ? '#ef4444' : (dev.status === 'danger' ? '#f87171' : (dev.status === 'warning' ? '#f59e0b' : '#4ade80')))};">
                                ${typeof dev.value === 'number' ? dev.value.toFixed(1) : dev.value} ${dev.unit}
                            </div>
...
function showMonitoringRMDetail(deviceId) {
    ...
    else if (device.status === 'critical') { statusText = '❗ Небезпечний'; statusClass = 'critical'; }
    ...
    break;
        case 'spectro':
            infoCards = `
                <div class="info-card"><div class="info-label">📊 Активність</div><div class="info-value" style="color:#fbbf24;">${typeof (device.data?.activity || device.value) === 'number' ? (device.data?.activity || device.value).toFixed(1) : device.value} ${device.unit}</div></div>
...
// ========== РЕЖИМ ЕКСПЕРТА ДЛЯ ГАММА-ДЕТЕКТОРА ==========
// ========== РЕЖИМ ЕКСПЕРТА ДЛЯ ГАММА-ДЕТЕКТОРА ==========
let expertCursorChannel = 473;
let expertCursorValue = 0;
let expertSpectrumChart = null;

function renderExpertMode(device) {
    const panel = document.getElementById('monitoringRMDetail');
    if (!panel) {
        ...
        <div class="expert-mode-container">
            <div class="expert-mode-header">
                <div class="expert-mode-title">
                    <span>⚡</span> РЕЖИМ ЕКСПЕРТА
                </div>
                <div class="expert-mode-buttons">
                    <button onclick="closeExpertMode()" style="background:#ef444422; border-color:#ef4444; color:#ef4444;">✕ ЗАКРИТИ</button>
                    <button onclick="refreshExpertSpectrum('${device.id}')" style="background:#4a9eff22; border-color:#4a9eff; color:#4a9eff;">🔄 ОНОВИТИ</button>
                </div>
            </div>

            <div class="expert-mode-grid">
                <!-- Ліва колонка - параметри -->
                <div>
                    <div class="expert-mode-params">
                        <div class="param-row">
                            <span class="param-label">Потужність дози, мкЗв/год</span>
                            <span class="param-value" style="color: ${gammaValue > 0.30 ? '#ef4444' : (gammaValue > 0.25 ? '#fbbf24' : '#4ade80')}">${dose.toFixed(2)}</span>
                        </div>
                        <div class="param-row">
                            <span class="param-label">Похибка, %</span>
                            <span class="param-value">${error}</span>
                        </div>
                        <div class="param-row">
                            <span class="param-label">Завантаження, імп./с</span>
                            <span class="param-value">${load}</span>
                        </div>
                        <div class="param-row">
                            <span class="param-label">Самоконтроль ЛГМ:</span>
                            <span class="param-value ok">✅ ОК</span>
                        </div>
                        <div class="param-row">
                            <span class="param-label">Самок-ль високочутл.дет.:</span>
                            <span class="param-value ok">✅ ОК</span>
                        </div>
                        <div class="param-row">
                            <span class="param-label">Тривалість накопичення, с</span>
                            <span class="param-value">${accumulationTime}</span>
                        </div>
                        <div class="param-row">
                            <span class="param-label">Завантаження, імп./с</span>
                            <span class="param-value">${load}</span>
                        ...
                        <div class="param-row">
                            <span class="param-label">Час початку</span>
                            <span class="param-value">${timeStr}</span>
                        </div>
                        <div class="param-row">
                            <span class="param-label">Дата початку</span>
                            <span class="param-value">${dateStr}</span>
                        </div>
                        <div class="param-row">
                            <span class="param-label">Температура, °C</span>
                            <span class="param-value">${temperature}</span>
                        </div>
                    </div>

                    <div class="expert-mode-controls">
                        <button class="primary" onclick="startExpertAccumulation('${device.id}')">📊 НОВЕ НАКОПИЧЕННЯ</button>
                        <button onclick="exportExpertData('${device.id}')">📤 ОБМІН</button>
                        <button class="danger" onclick="pauseExpertAccumulation('${device.id}')">⏸ ПАУЗА</button>
                    </div>
                </div>

                <!-- Права колонка - спектр -->
                <div>
                    <div class="expert-mode-spectrum">
                        <canvas id="expertSpectrumCanvas"></canvas>
                        <div class="expert-mode-spectrum-labels">
                            <span>0</span>
                            <span>100</span>
                            <span>200</span>
                            <span>300</span>
                    ...
                    <div class="expert-mode-cursor">
                        <div class="cursor-item">
                            <span class="cursor-label">Курсор:</span>
                            <span class="cursor-value">Канал ${expertCursorChannel}</span>
                        </div>
                        <div class="cursor-item">
                            <span class="cursor-label">Значення</span>
                            <span class="cursor-value" id="expertCursorValue">${expertCursorValue}</span>
                        </div>
                    </div>

                    <div class="expert-mode-device-info">
                        <span>📡 ${device.name}</span>
                        <span>🔢 ${device.serial || 'N/A'}</span>
                        <span>📊 Статус: <span style="color: ${gammaValue > 0.30 ? '#ef4444' : (gammaValue > 0.25 ? '#fbbf24' : '#4ade80')}">${statusText}</span></span>
                    </div>
                </div>
            </div>
        </div>
...
function drawExpertSpectrum(spectrumData) {
    ...
    // Дані спектру
    const data = spectrumData.map(p => p.counts);
    ...
    // Малюємо спектр як лінію
    ctx.beginPath();
    ctx.strokeStyle = '#00ff41';
    ctx.lineWidth = 1.5;
    ctx.shadowColor = '#00ff4144';
    ctx.shadowBlur = 8;
    
    for (let i = 0; i < data.length; i++) {
        const x = (i / data.length) * width;
        const y = height - (data[i] / maxVal) * height * 0.9 - 5;
        if (i === 0) {
            ctx.moveTo(x, y);
        } else {
            ctx.lineTo(x, y);
    ...
    // Заливка під кривою
    const lastX = width;
    const lastY = height - (data[data.length-1] / maxVal) * height * 0.9 - 5;
    ...
    // Піки з підписами
    const peaks = [
        { channel: 441, label: 'Cs-137' },
        { channel: 782, label: 'Co-60' },
        { channel: 888, label: 'Co-60' }
    ];
    
    peaks.forEach(peak => {
        const idx = Math.min(Math.round(peak.channel / (2048 / data.length)), data.length - 1);
        const x = (idx / data.length) * width;
        const y = height - (data[idx] / maxVal) * height * 0.9 - 5;
        ...
        if (data[idx] > 50) {
            ...
            ctx.fillText(peak.label, x, y - 10);
        }
    });
    
    // Лінія курсора
    const cursorIdx = Math.min(Math.round(expertCursorChannel / (2048 / data.length)), data.length - 1);
    const cursorX = (cursorIdx / data.length) * width;
    const cursorY = height - (data[cursorIdx] / maxVal) * height * 0.9 - 5;
    ...
    // Оновлюємо значення курсора
    expertCursorValue = Math.round(data[cursorIdx]);
    const cursorDisplay = document.getElementById('expertCursorValue');
    if (cursorDisplay) {
        cursorDisplay.textContent = expertCursorValue;
    }
}

function refreshExpertSpectrum(deviceId) {
    const device = allDevices.find(d => d.id === deviceId);
    if (!device) return;
    
    // Генеруємо новий спектр
    const spectrum = generateSpectrumData();
    device.data = device.data || {};
    device.data.spectrum = spectrum;
    
    // Оновлюємо значення
    const gammaValue = typeof device.value === 'number' ? device.value : 0;
    device.value = Number((gammaValue + (Math.random() - 0.5) * 0.05).toFixed(2));
    device.value = Math.max(0, Math.min(0.50, device.value));
    
    renderExpertMode(device);
    addNotification('info', '🔄 Спектр оновлено', `Оновлено дані для ${device.name}`);
}
...
function closeExpertMode() {
    ...
    // Вимикаємо режим експерта для поточного приладу
    if (currentMonitoringDevice) {
        currentMonitoringDevice.expertMode = false;
    }
}

function startExpertAccumulation(deviceId) {
    const device = allDevices.find(d => d.id === deviceId);
    if (!device) return;
    addNotification('info', '📊 Нове накопичення', `Розпочато нове накопичення для ${device.name}`);
    refreshExpertSpectrum(deviceId);
}

function pauseExpertAccumulation(deviceId) {
    const device = allDevices.find(d => d.id === deviceId);
    if (!device) return;
    addNotification('warning', '⏸ Пауза', `Накопичення призупинено для ${device.name}`);
}

function exportExpertData(deviceId) {
    const device = allDevices.find(d => d.id === deviceId);
    if (!device) return;
    exportDeviceExcel(deviceId);
}

function toggleExpertMode(deviceId) {
    console.log('toggleExpertMode called for:', deviceId);
    const device = allDevices.find(d => d.id === deviceId);
    if (!device) {
    ...
    device.expertMode = !device.expertMode;
    console.log('Expert mode:', device.expertMode);
    
    if (device.expertMode) {
        addNotification('info', '⚡ Режим експерта', `Увімкнено режим експерта для ${device.name}`);
        renderExpertMode(device);
    } else {
        addNotification('info', '📊 Стандартний режим', `Виключено режим експерта для ${device.name}`);
        showMonitoringRMDetail(deviceId);
    }
}
// ========== ГОЛОВНА ФУНКЦІЯ ==========
                    ...
                    <div id="window-devices" class="window">
                        ...
                        <div class="filter-bar">
                            <select id="devFilterType" class="filter-select" onchange="renderFilteredDevices()">
                                <option value="all">Всі типи</option>
                                <option value="gamma">>Блоки детектування ПАЕД гамма-випромінювання</option>
                                <option value="spectro">Спектрометричні блоки детектування</option>
                                <option value="vfu">Повітряно-фільтрувальні установки (ПФУ) атмосферного повітря</option>
                                <option value="weather">Метеостанції</option>
                            </select>
                        ...
                        <div class="window-content">
                            <div class="filter-bar" style="flex-wrap:wrap; gap:10px;">
                                ...
                                <select id="alarmFilterType" class="filter-select" onchange="applyAlarmFilters()">
                                    <option value="all">📡 Всі типи</option>
                                    <option value="gamma">Блок детектування ПАЕД гамма-випромінювання</option>
...
function renderWorkplaces() {
    ...
    container.innerHTML = filteredWorkplaces.map(wp => {
        ...
        if (typeFilter !== 'all') equipmentList = equipmentList.filter(e => e.type === typeFilter);
        if (statusFilter !== 'all') equipmentList = equipmentList.filter(e => e.status === statusFilter);
        return `<div class="workplace-card"><div class="workplace-header" onclick="toggleWorkplace('wp-' + wp.id)"><div><div class="workplace-name">🏭 ${wp.name}</div><div class="workplace-location">📍 ${wp.location} (${wp.area})</div></div></div></div>`;
    }).join('');
}
...
function showDeviceDetail(deviceId) {
    ...
    if (device.type === 'gamma') {
        const gammaValue = typeof device.value === 'number' ? device.value : 0;
        if (gammaValue > 0.30) {
            statusText = '❗ Критично!';
            statusClass = 'critical';
        } else if (gammaValue > 0.25) {
            statusText = '⚠️ Попередження!';
            statusClass = 'warning';
        } else {
            statusText = '✅ Норма';
            statusClass = 'normal';
    ...
    panel.innerHTML = `
        ...
        <div class="logs-panel">
            ...
            <div id="deviceLogs">${device.logs?.slice().reverse().slice(0,15).map(log => '<div class="log-entry ' + log.type + '">[' + new Date(log.timestamp).toLocaleString() + '] ' + log.type.toUpperCase() + ': ' + log.message + '</div>').join('')}</div>
        </div>
    `;
...
function openWindow(id) {
    ...
    const titles = { map: 'МАПА', monitoring: 'МОНІТОРИНГ', devices: 'ПРИЛАДИ', users: 'КОРИСТУВАЧІ', permissions: 'ПРАВА ДОСТУПУ', alarms: 'НАЛАШТУВАННЯ ТРИВОГ' };
...
function selectLanguage(langCode, flag, langName) {
    ...
    const titles = {
        uk: { map: 'МАПА', monitoring: 'МОНІТОРИНГ', devices: 'ПРИЛАДИ', users: 'КОРИСТУВАЧІ', permissions: 'ПРАВА ДОСТУПУ', alarms: 'НАЛАШТУВАННЯ ТРИВОГ', logout: 'ВИХІД' },
...
// Додати до існуючого списку глобальних функцій
window.renderExpertMode = renderExpertMode;
window.closeExpertMode = closeExpertMode;
window.refreshExpertSpectrum = refreshExpertSpectrum;
window.startExpertAccumulation = startExpertAccumulation;
window.pauseExpertAccumulation = pauseExpertAccumulation;
window.exportExpertData = exportExpertData;
window.toggleExpertMode = toggleExpertMode;
window.drawExpertSpectrum = drawExpertSpectrum;
